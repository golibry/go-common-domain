...
func (s *MoneyTestSuite) TestMoneyArithmetic() {
	...
	s.Run("addition with same currency", func() {
	...
	s.Run("addition with different currency fails", func() {
		_, err := money1.Add(money3)
		s.Error(err)
	})

	s.Run("subtraction with same currency", func() {
	...
	s.Run("subtraction with different currency fails", func() {
		_, err := money1.Subtract(money3)
		s.Error(err)
	})

	s.Run("subtraction resulting in negative fails", func() {
		_, err := money2.Subtract(money1)
		s.Error(err)
		s.True(errors.Is(err, ErrNegativeAmount))
	})

	s.Run("multiplication", func() {
	...
	s.Run("multiplication by negative factor fails", func() {
...
func (s *MoneyTestSuite) TestItCanBuildNewMoneyFromValidJSON() {
	jsonData := `{"amount":"100.50","currency":"USD"}`
	
	money, err := NewMoneyFromJSON([]byte(jsonData))
	s.NoError(err)
	s.Equal("100.5", money.Amount().String())
	s.Equal("USD", money.Currency().String())
}

func (s *MoneyTestSuite) TestItFailsToBuildNewMoneyFromInvalidJSON() {
	testCases := []struct {
		name     string
		jsonData string
	}{
		...
		{
			name:     "invalid amount in JSON",
			jsonData: `{"amount":"abc","currency":"USD"}`,
		},
		{
			name:     "negative amount in JSON",
			jsonData: `{"amount":"-100.50","currency":"USD"}`,
		},
	}
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			_, err := NewMoneyFromJSON([]byte(tc.jsonData))
			s.Error(err)
		})
	}
}